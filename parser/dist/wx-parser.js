var Page = (function (Vue) {
	...
	const handleWxFor = (attrs) => {
	    ...
	    for (let i = attrs.length - 1; i >= 0; i--) {
	        ...
	        if (forMap[name]) {
	            forMsg[name] =
	                name === ForList ? attrs[i].value.match(propRE$1)[1] : attrs[i].value;
	            removeItem(attrs, i);
	        }
	    }
	    const attr = {
	...
	const MapEventWXToWeb = (attrs) => {
	    for (let i = 0; i < attrs.length; i++) {
	        const e = attrs[i];
	        if (!e.name.startsWith('bind'))
	            continue;
	        const name = e.name.replace(onRE$1, '');
	        e.name = `@${eventMap[name] ? eventMap[name] : name}`;
	    }
	};
	const MapPropWXToWeb = (attrs) => {
	    let isFor = false;
	    for (let i = 0; i < attrs.length; i++) {
	        const e = attrs[i];
	        ...
	        if (e.name === 'wx:for') {
	            ...
	            attrs.splice(i, 1, Object.assign(Object.assign({}, e), { name: ':key', value: propRE.test(e.value) ? RegExp.$1 : e.value }));
	        }
	        else if (propRE.test(value)) {
	            attrs.splice(i, 1, Object.assign(Object.assign({}, e), { name: ':' + e.name, value: RegExp.$1 }));
	        }
	        else if (propRE2.test(value)) {
	            const _val = value.replace(propRE2, (_, b) => {
	                return `\$\{${b}\}`;
	            });
	            attrs.splice(i, 1, Object.assign(Object.assign({}, e), { name: ':' + e.name, value: '`' + _val + '`' }));
	        }
	    }
	    if (isFor) {
	        handleWxFor(attrs);
	...
	function processAttrs(el) {
	  const list = el.attrsList;
	  let i, l, name, rawName, value, modifiers, isDynamic;
	 ...
	 * @returns
	 */
	const mapElement = (tag) => {
	    return elementMap[tag] || 'wx-' + tag;
	};

	const RE = /[\n\r]+/g; // 匹配换行符
	const creatTextElement = (texts) => {
	    if (!texts)
	        return '';
	    return texts.reduce((html, ast) => {
	        if (ast.type !== 3 || typeof ast.text !== 'string')
	            return html;
	...
	(function (IElementType) {
	    IElementType[IElementType["tag"] = 1] = "tag";
	    IElementType[IElementType["expression"] = 2] = "expression";
	    IElementType[IElementType["text"] = 3] = "text";
	})(IElementType || (IElementType = {}));

	const attrsToString = (attrs) => {
	    if (attrs == null)
	        return '';
	    return Object.keys(attrs).reduce((pre, next) => {
	        return pre + `${next}="${attrs[next]}" `;
	    }, '');
	};

	const tagElement = (ast) => {
	 ...
	 */
	const handleWXML = (wxml, options) => {
	    return generate(parse(wxml, options));
	};

	const rpxRE = /\b((\d+)rpx)\b/;

	const wxssToCss = (wxss) => {
	    // 替换 rpx 为 vw 表示
	    let m;
	...
	const Page = (options) => {
	  ...
	  Vue__default['default'].use(WxUI);

	  new Vue__default['default']({
	    el: '#app',
	    template: handleWXML(el.innerHTML, {}),
	    data() {
	      return options.data || {};
	    },
	    beforeCreate() {
